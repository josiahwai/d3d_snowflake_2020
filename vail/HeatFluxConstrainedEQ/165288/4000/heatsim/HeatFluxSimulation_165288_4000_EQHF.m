...
fOBL = 0.60; % fraction of power to outboard divertor
...
for ii = 1:length(psiSOL)
    ...
    if psixSL > psixPL % SFD-Plus
            ...
            [L, r, z] = calcConnectionLength_wai(r0, z0, psizr, rg, zg, ...
...
frad_SP1 = 0.80; % radiated power fraction for SP1
...
for ii = 1:length(idxpsi)-1
    ...
    qdiv_OL = q_parallel_IL(idx)*(1-frad_SP1);
...
frad_SP2 = 0.80; % radiated power fraction for SP2
...
for ii = 1:length(idxpsi)-1
    ...
    qdiv_OL = q_parallel(idx)*(1-frad_SP2);
    psiDivFine = linspace(psiSOL(idx), psiSOL(idx+1), 100);
...
frad_SP3 = 0.80; % radiated power fraction for SP3
...
for ii = 1:length(idxpsi)-1
    ...
    qdiv_OL = q_parallel_OL(idx)*(1-frad_SP3);
    psiDivFine = linspace(psiSOL(idx), psiSOL(idx+1), 100);
...
for ii = 1:length(psiDivSP3)
    ...
    switch size(rz,1)
        ...
        case 3
            rzDiv_SP3(ii,1) = rz(3,1);
            rzDiv_SP3(ii,2) = rz(3,2);
        case 4
...
optionsOuter.StartPoint = [2*max(qdiv_perp_SP2) 0.01 lambdaQ sSP 0];
   
fitEich_SP2 = fit(sDiv_SP2, qdiv_perp_SP2, ftOuter, 'problem', fExp, ...
    optionsOuter);
...
if saveit
    save(['HeatFluxSimulation_' int2str(shot) '_' int2str(time) ...
        '_EQHF.mat'], 'HeatFluxSimulation')
end

rmpath(genpath('/u/jwai/d3d_snowflake_2019'));
