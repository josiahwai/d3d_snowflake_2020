...
for ii = 1:numSegs
    ...
    rlim = linspace(rS, rE, nFine);
    zlim = linspace(zS, zE, nFine);
    ...
    [~, idxs] = findpeaks(-diffs);
    
    if diffs(1) < 2.5e-4
        idxs = [1 idxs];
    end
    if diffs(nFine) < 2.5e-4
        idxs = [idxs nFine];
    end
...
% Sort strike points in order of increasing major radius

spRZ = uniquetol(sortrows(spRZ(rowKeepIdx,:),1), 'ByRows', 1e-12);

% Sort strike points with same major radius in order of decreasing vertical

[~, ia] = unique(spRZ(:,1));

idxRepeats = setdiff(1:size(spRZ(:,1),1), ia);

for ii = 1:length(idxRepeats)
    
    idx  = idxRepeats(ii);
    
    if find(ia < idx, 1, 'last' ) == 1
        spRZ(1:idx,:) = sort(spRZ(1:idx,:), 'descend');
    end
