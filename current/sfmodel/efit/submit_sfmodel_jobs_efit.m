...
for iTime = 1:length(t_sim)
  ...
  batchscript_fn = [root 'sfmodel/efit/sfmodel_efit.sbatch'];   
  if sim_sfm  
    jobdir = [root 'sfmodel/jobs/sfm_efit2/' num2str(shot) '_sfm/' num2str(time_ms) '/'];
  elseif sim_sfp && constrain_sp
    jobdir = [root 'sfmodel/jobs/sfp_constrain_sp_efit/' num2str(shot) '_sfp_sp/' num2str(time_ms) '/'];    
  elseif sim_sfp && ~constrain_sp
    jobdir = [root 'sfmodel/jobs/sfp_efit/' num2str(shot) '_sfp/' num2str(time_ms) '/'];
  end
  ...
  jobscript = [root 'sfmodel/efit/sfmodel_efit.m'];
