...
ef.rsp(end) = ef.rsp(end) - .02;
ef.ssp(end) = ef.ssp(end) - .02;


% plot heat flux
plot(sir,qirN,'k','linewidth',1.5)
plot(s, qN, 'color', blue, 'linewidth', 1.5)
...
for i = [1 2 4]
  ...
  text(min(ef.ssp(i), ssp(i))*100 - 8, 0.6, ['SP' num2str(i)], ...
    'fontweight', 'bold')
end
...
% plot heat flux
plot(s, qN, 'color', orange, 'linewidth', 1.5)
