...
i_sfp = find(i_snowtype == 1 & ismember(shots,shotlist));
i_sfm = find(i_snowtype == 3 & ismember(shots,shotlist));
...
figure
hold on
scatter(dxp_sfp(:,1), dxp_sfp(:,3), 15, 'markerfacecolor', orange)
scatter(dxp_sfm(:,1), dxp_sfm(:,3), 15, 'markerfacecolor', skyblue)
scatter(dxp_sfp(:,2), dxp_sfp(:,4), 15, 'markerfacecolor', blugreen)
scatter(dxp_sfm(:,2), dxp_sfm(:,4), 15, 'markerfacecolor', yellow)
...
scatter(dxp_sfm(:,2), dxp_sfm(:,4))


subplot(2,2,3)
hold on
title('Snow-Plus Primary X-Pt')
scatter(dxp_sfp(:,1), dxp_sfp(:,3))
xlabel('\Delta r [cm]')
ylabel('\Delta z [cm]')
